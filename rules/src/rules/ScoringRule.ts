...
export class ScoringRule extends MaterialRulesPart {
  ...
  onRuleEnd<RuleId extends number>(_move: RuleMove<number, RuleId>): MaterialMove<number, number, number>[] {
    ...
    if (this.remind(Memory.Bid, bid) === Bid.GuardAgainstTheKitty) {
      points = pointsTricks
    ...
    for (const player of this.game.players) {
      ...
      if (this.game.players.length === 3 || this.game.players.length === 4) {
        if (player !== preneur) {
          ...
          playerScore = score * (this.game.players.length - 1)
        }
      }
      ...
      this.addSummary(round, player, playerScore)
      this.memorize(Memory.Score, (actualScore: number) => actualScore + playerScore, preneur)
    }
  ...
  addSummary = (round: number, player: number, score: number) => {
    this.memorize(Memory.RoundSummary, (r = []) => {
      ...
      rounds[round - 2].push({ player, score })
      return rounds
    })
  }
}
