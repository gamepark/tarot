...
import { PoigneeRule } from '@gamepark/tarot/rules/PoigneeRule'
...
export class TarotCardDescription extends CardDescription {
  ...
  canShortClick(move: MaterialMove, context: ItemContext): boolean {
    return isMoveItemType(MaterialType.Card)(move) && move.itemIndex === context.index && move.location.type === LocationType.Table
  }

  getShortClickLocalMove(context: ItemContext) {
    const { rules, player, index } = context
    if (!player) return
    if (rules.game.rule?.id !== RuleId.Poignee || player !== rules.game.rule?.player) return
    const material = rules.material(MaterialType.Card).index(index)
    const item = material.getItem()!
    if (item.location.player !== player) return
    const poigneeMoves = new PoigneeRule(rules.game).poigneeMoves
    const selectable = poigneeMoves.some((move) => isMoveItemTypeAtOnce(MaterialType.Card)(move) && move.indexes.includes(index))
    if (!selectable) return
    if (item.selected) {
      return rules.material(MaterialType.Card).index(index).unselectItem()
    } else {
      return rules.material(MaterialType.Card).index(index).selectItem()
    }
